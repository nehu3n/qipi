...
pub fn generate_lockfile(
    ...
    let toml_string =
        toml::to_string(&lockfile).map_err(|e| format!("Error serializing lockfile: {}", e))?;
    ...
    let mut file =
        File::create(&lockfile_path).map_err(|e| format!("Error creating lockfile: {}", e))?;

    file.write_all(lockfile_content.as_bytes())
        .map_err(|e| format!("Error writing lockfile: {}", e))?;

    Ok(())
}

pub fn load_lockfile(destination: &str) -> Result<Lockfile, String> {
    let lockfile_path = format!("{}/qipi.toml", destination);

    if !Path::new(&lockfile_path).exists() {
        return Err("Lockfile not found.".to_string());
    }

    let content = std::fs::read_to_string(lockfile_path)
        .map_err(|e| format!("Error reading lockfile: {}", e))?;

    let lockfile: Lockfile =
        toml::from_str(&content).map_err(|e| format!("Error parsing lockfile: {}", e))?;
